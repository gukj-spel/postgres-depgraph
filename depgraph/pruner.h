...
class PruningFunc {
public:
    virtual void prune_in_channels(ONNX_MODULE* module, const std::vector<int>& idxs) = 0;
    virtual void prune_out_channels(ONNX_MODULE* module, const std::vector<int>& idxs) = 0;
...
public:
    void prune_in_channels(ONNX_MODULE* module, const std::vector<int>& idxs) override {
        // 实现拼接层 override的剪枝逻辑
        // 这里需要根据具体的模块类型实现不同的剪枝方式
    }
    void prune_out_channels(ONNX_MODULE* module, const std::vector<int>& idxs) override {
        // 实现拼接层的剪枝逻辑
        // 这里需要根据具体的模块类型实现不同的剪枝方式
    }
};
